...
if (location.pathname === "/site-details") {
    ...
    document.querySelector(".Content__siteDetails--placeExtra-button button").addEventListener("click", ()=>{
        document.querySelector(".Content__siteDetails--placeExtra-modal").classList.add("allExtraModalActive")
        document.querySelector("body").style.overflow = "hidden";
    })
    document.querySelector(".siteDetails__placeEquipment--allExtra-title svg").addEventListener("click", ()=>{
        document.querySelector(".Content__siteDetails--placeExtra-modal").classList.remove("allExtraModalActive")
        document.querySelector("body").style.overflow = "initial";
    })
...
if(location.pathname == "/faqs"){
    ...
    $('#faqsSearch').keyup(function(){
        ...
        $.ajax({
            url: '/buscarfaq',
            method: 'GET',
            data: { term: term, lang: lang }, // Incluir el idioma seleccionado en la solicitud
            success: function(response){
                ...
                if (response && response.faqs && response.faqs.length > 0) {
                    $.each(response.faqs, function(index, faq) {
                        $('#result').append(`
                            <div class="Content__faqs--faq">
                                <button class="accordion">${faq.Pregunta.Contenido}<i class="fa-solid fa-chevron-down"></i></button>
                                <div class="panel">
                                    <p>${faq.Respuesta.Contenido}</p>
                                </div>
                            </div>
                        `);
                        applyAccordionEvents();
                    ...
                    $('#result').html('No se encontraron resultados');
                }
            }
        });
    });
